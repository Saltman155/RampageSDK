...
class CPPLibBuild(build_clib):
    ...
    def run(self) -> None:
        ...
        rampage_dir = os.path.join(BASE_DIR, build_py.build_lib, build_py.get_package_dir("rampage"))
        if not os.path.exists(rampage_dir):
            os.makedirs(rampage_dir)
...
setup(
    ...
    keywords="rampage",
    ext_modules=[Extension("rampage._C", sources=[])],
    author="Ascend Contributors",
    libraries=[("rampage", {"sources": []})],
    cmdclass={
